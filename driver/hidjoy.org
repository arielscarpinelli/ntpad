    ...
    )
{
    ...
    {   
        ...
        {   { HID_USAGE_PAGE_GENERIC, HID_USAGE_GENERIC_X} , 
            ...
            { HID_USAGE_PAGE_GENERIC, HID_USAGE_GENERIC_RZ} } 
    };
...
#define ITEM_PADDING        0x01 /* Constant (nothing else applies) */
     ...
     * Now the buttons 
     */
    for( Idx = 0x0; Idx < MAX_BUTTONS; Idx++ )
    {
    ...
    /* Force FeedBack Stars Here!!!!!!!!!!!!!!!!!! */    
    NEXT_BYTE(pucReport,  HIDP_GLOBAL_USAGE_PAGE_1);
    NEXT_BYTE(pucReport,  0x0F);

    NEXT_BYTE(pucReport,  HIDP_LOCAL_USAGE_1);
    NEXT_BYTE(pucReport,  HIDP_MAIN_COLLECTION);
    NEXT_BYTE(pucReport,  0x02);

      NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
      NEXT_BYTE(pucReport, 0x22);
      NEXT_BYTE(pucReport, HIDP_LOG_MAX_1);
      NEXT_BYTE(pucReport, 127);
      NEXT_BYTE(pucReport, HIDP_GLOBAL_REPORT_SIZE);
      ...
      NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
      NEXT_BYTE(pucReport, 0x25);
      NEXT_BYTE(pucReport, HIDP_MAIN_COLLECTION);
      NEXT_BYTE(pucReport, 0x02);
        NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
        NEXT_BYTE(pucReport, 0x26);
        NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
        NEXT_BYTE(pucReport, 0x27);
        NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
        NEXT_BYTE(pucReport, 0x31); 
        NEXT_BYTE(pucReport, HIDP_LOG_MIN_1);
        NEXT_BYTE(pucReport, 1);  
        NEXT_BYTE(pucReport, HIDP_LOG_MAX_1);
        NEXT_BYTE(pucReport, 0x10);
        NEXT_BYTE(pucReport, HIDP_GLOBAL_REPORT_SIZE);
      ...
      NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
      NEXT_BYTE(pucReport, 0x50);
      NEXT_BYTE(pucReport, HIDP_LOCAL_USAGE_1);
      NEXT_BYTE(pucReport, 0x54);
      NEXT_BYTE(pucReport, HIDP_LOG_MIN_1);
      NEXT_BYTE(pucReport, 0);  
      NEXT_BYTE(pucReport, HIDP_LOG_MAX_1);
      NEXT_LONG(pucReport, 10000);
      NEXT_BYTE(pucReport, HIDP_PHY_MAX_1);
      NEXT_LONG(pucReport, 10000);
      NEXT_BYTE(pucReport, HIDP_GLOBAL_REPORT_SIZE);
      NEXT_BYTE(pucReport, 8);
      NEXT_BYTE(pucReport, HIDP_GLOBAL_UNIT_2);
      NEXT_BYTE(pucReport, 0x30);
      NEXT_BYTE(pucReport, 0x10);      
      NEXT_BYTE(pucReport, HIDP_GLOBAL_UNIT_EXP_1);
      NEXT_BYTE(pucReport, 0x0D);
