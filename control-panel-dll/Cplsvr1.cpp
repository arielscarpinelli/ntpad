...
#include "cplsvr1.h"
#include "pages.h"
typedef BOOL (WINAPI *ACTFUNC)(HANDLE);
...
static  CPLPAGEINFO     grgcpInfo[NUMPAGES] = {
    { TEXT(""), IDS_MAIN, MAKEINTRESOURCEW(IDD_CPLSVR1_PAGE1), CPLSVR1_Page1_DlgProc },
    { TEXT(""),IDS_MAPPING, MAKEINTRESOURCEW(IDD_GENERIC_REMAP), CPLSVR1_Page2_DlgProc },
    { TEXT(""),IDS_FEEDBACK, MAKEINTRESOURCEW(IDD_CPLSVR1_FEEDBACK), CPLSVR1_Page3_DlgProc },
    { TEXT(""),IDS_CONFIG, MAKEINTRESOURCEW(IDD_CPLSVR1_CONFIG), CPLSVR1_Page4_DlgProc },
    { TEXT(""),IDS_ABOUT, MAKEINTRESOURCEW(IDD_CPLSVR1_ABOUT), CPLSVR1_Page5_DlgProc }};
UINT CALLBACK PPS(HWND hwnd,
                                UINT uMsg,
                                ...
                                )
{
        ...
        HMODULE hm;
        ACTFUNC func; 
        if(ppsp->dwSize < sizeof(PROPSHEETPAGE))
           return TRUE;
        hm = GetModuleHandle(TEXT("kernel32.dll"));
        func = (ACTFUNC)GetProcAddress(hm, "GetCurrentActCtx");
        if(!func)
          return TRUE;
        func(&a);
        ...
        {
           ppsp->hActCtx = a;
           ppsp->dwFlags |= PSP_USEFUSIONCONTEXT;
        }
        return TRUE;
}
...
CDIGameCntrlPropSheet_X::~CDIGameCntrlPropSheet_X(void)
{
    ...
    if(NULL != m_ConfighKey)
    {
        RegCloseKey(m_ConfighKey);
        m_ConfighKey = NULL;
    };
...
HRESULT CDIGameCntrlPropSheet_X::Initialize(void)
{
    ...
    if(RegCreateKeyEx(HKEY_LOCAL_MACHINE, (LPCWSTR) TEXT("System\\CurrentControlSet\\Control\\MediaProperties\\PrivateProperties\\Joystick\\NTPAD"), 0, NULL,
                      0, KEY_ALL_ACCESS, NULL, &m_ConfighKey, &lCreated) != ERROR_SUCCESS)
        return S_FALSE;

    LCID locale;
    DWORD Size = sizeof(DWORD);
    locale = GetThreadLocale();
    RegQueryValueEx(m_ConfighKey, (LPCWSTR)TEXT("Idioma"), 0, &lCreated, (LPBYTE)&locale, &Size);
    ...
    for (BYTE i = 0; i < NUMPAGES; i++)
    {
        ...
        m_pdigcPageInfo[i].fProcFlag     = TRUE;
        m_pdigcPageInfo[i].fpPrePostProc = (DLGPROC)PPS;
        m_pdigcPageInfo[i].fIconFlag     = TRUE;
        m_pdigcPageInfo[i].lpwszPageIcon = NULL;
        m_pdigcPageInfo[i].hInstance     = ghInst;
        m_pdigcPageInfo[i].lParam        = (LPARAM)this;
        // the following data is unique to each page
        LoadString(ghInst, grgcpInfo[i].strnameid, (LPTSTR)&grgcpInfo[i].name, 25);
        m_pdigcPageInfo[i].lpwszPageTitle    = (LPTSTR)&grgcpInfo[i].name;
    ...
    m_digcSheetInfo.fSheetIconFlag      = FALSE;
    m_digcSheetInfo.lpwszSheetIcon      = NULL; //MAKEINTRESOURCEW(IDI_WINFLAG);
...
//===========================================================================
STDMETHODIMP CDIGameCntrlPropSheet_X::SetHIDHandle(HANDLE hHandle)
{
...
//===========================================================================
STDMETHODIMP CDIGameCntrlPropSheet_X::GetHIDHandle(HANDLE *hHandle)
{
...
//===========================================================================
STDMETHODIMP CDIGameCntrlPropSheet_X::GetConfighKey(HKEY *hKey)
{
